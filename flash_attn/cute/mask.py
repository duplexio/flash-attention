...
class AttentionMask:
    ...
    col_limit: Optional[cute.Tensor] = None  # (total_q,) int32
    offset_q: Int32 = 0
    ...
    def apply_mask(
        ...
        mask_col_limit: cutlass.Constexpr[bool] = False,
        ...
        if const_expr(not mask_causal and not mask_local and not mask_col_limit and mask_mod is None):
            ...
            not mask_causal and not mask_local and not mask_col_limit and mask_mod is not None
        ...
        elif const_expr(mask_col_limit):
            # Release mask: per-Q-row column limits from col_limit tensor.
            # Optional window_size_left narrows the visible range to
            # [max(0, col_limit[i] - window_size_left), col_limit[i]).
            if const_expr(not self.swap_AB):
                ...
                for r in cutlass.range(cute.size(tScS_mn.shape[0]), unroll_full=True):
                    ...
                    visible_kv = self.col_limit[self.offset_q + row_idx]
                ...
                for c in cutlass.range(cute.size(tScS_mn.shape[1]), unroll_full=True):
                    ...
                    # In the transposed (backward) view, we need row_limit
    ...
    def apply_mask_sm100(
        ...
        if const_expr(not mask_causal and not mask_local and not mask_col_limit and mask_mod is None):
        ...
        elif const_expr(not mask_causal and not mask_local and not mask_col_limit and mask_mod is not None):
            ...
            # Release mask: per-thread row has its own col_limit from col_limit tensor
