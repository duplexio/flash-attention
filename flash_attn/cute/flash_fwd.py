...
class FlashAttentionForwardSm80(FlashAttentionForwardBase):
    ...
    def kernel(
        ...
        block_info = BlockInfo(
            ...
            window_size_right,
            qhead_per_kvhead_packgqa=self.qhead_per_kvhead if const_expr(self.pack_gqa) else 1,
            has_kv_seqused=self.has_kv_seqused,
        )
        ...
        if const_expr(not self.has_kv_seqused):
            n_block_min, n_block_max = block_info.get_n_block_min_max(seqlen, m_block)
        else:
            if const_expr(window_size_left is not None):
                n_block_min = block_info.get_n_block_min_kv_seqused_window(
                    mKvSeqused, seqlen, m_block, window_size_left
                )
            else:
                n_block_min = Int32(0)
            n_block_max = block_info.get_n_block_max_kv_seqused(
                mKvSeqused, seqlen, m_block
            )
        ...
        mask = AttentionMask(
            ...
            offset_q=seqlen.offset_q if const_expr(self.has_kv_seqused) else 0,
        )
        mask_fn = partial(
            mask.apply_mask,
            batch_idx=batch_size,
        ...
        if const_expr(self.has_kv_seqused):
            n_block_min_kv_seqused = block_info.get_n_block_min_kv_seqused_mask(
                mKvSeqused, seqlen, m_block, n_block_min
            )
            for n_tile in cutlass.range(n_block_max - 1 - n_block_min_kv_seqused, unroll=1):
                n_block = n_block_max - 2 - n_tile
                compute_one_n_block(
                    n_block,
                    smem_pipe_read,
                ...
                smem_pipe_read = self.advance_pipeline(smem_pipe_read)
                smem_pipe_write = self.advance_pipeline(smem_pipe_write)
            n_block = cutlass.min(n_block, n_block_min_kv_seqused)
