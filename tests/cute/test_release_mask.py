...
# Tests for col_limit cross-attention support (forward + backward).
...
pytestmark = pytest.mark.skipif(
    ...
    reason="col_limit not supported on this GPU arch",
)


def attention_col_limit_ref(q, k, v, col_limit, softmax_scale=None, window_size_left=None):
    ...
    right = col_limit.unsqueeze(1)
    if window_size_left is not None:
        left = (col_limit - window_size_left).clamp(min=0).unsqueeze(1)
...
def generate_col_limit(seqlens_q, seqlens_k, device, min_visible=0):
    """Generate a random monotonically non-decreasing col_limit."""
    parts = []
...
def test_col_limit_fwd(seqlens_q, seqlens_k, d, mha_type, dtype):
    ...
    if is_fake_mode():
        col_limit = torch.zeros(total_q, dtype=torch.int32, device=device)
    else:
        col_limit = generate_col_limit(seqlens_q, seqlens_k, device)

    out, lse = flash_attn_varlen_func(
        q, k, v,
        cu_seqlens_q=cu_seqlens_q, cu_seqlens_k=cu_seqlens_k,
        max_seqlen_q=max(seqlens_q), max_seqlen_k=max(seqlens_k),
        causal=False, return_lse=True, col_limit=col_limit,
    )
    ...
    for sq, sk in zip(seqlens_q, seqlens_k):
        rm_batch = col_limit[offset_q : offset_q + sq]
        out_b, lse_b = attention_col_limit_ref(
    ...
    # Rows with col_limit=0 produce NaN (all -inf scores); exclude from comparison
    valid = col_limit > 0
...
@maybe_fake_tensor_mode(USE_FAKE_TENSOR)
def test_col_limit_bwd(seqlens_q, seqlens_k, d, mha_type, dtype):
    if SM120_NO_GQA and mha_type == "gqa":
        pytest.skip("SM120 varlen + GQA is pre-existing broken")
    device = "cuda"
    nheads = 8
    ...
    for sq, sk in zip(seqlens_q, seqlens_k):
        ...
        rm_b = col_limit[offset_q:offset_q+sq]
        dout_b = dout[offset_q:offset_q+sq].float()

        out_b, _ = attention_col_limit_ref(q_b, k_b, v_b, rm_b)
    ...
    # NaN can appear in gradients when col_limit=0 (all -inf scores)
...
@maybe_fake_tensor_mode(USE_FAKE_TENSOR)
def test_col_limit_all_visible(seqlens_q, seqlens_k, d, dtype):
    """col_limit with all KV visible should match non-causal attention."""
    device = "cuda"
    nheads = 4
    total_q = sum(seqlens_q)
    total_k = sum(seqlens_k)
...
# ============================================================================
# Sliding window + col_limit tests
# ============================================================================
...
@maybe_fake_tensor_mode(USE_FAKE_TENSOR)
def test_col_limit_window_fwd(seqlens_q, seqlens_k, d, window_size_left, dtype):
    """Forward: col_limit + window_size_left vs materialized reference."""
    if SM120_NO_GQA:
    ...
    if is_fake_mode():
        ...
        col_limit = generate_col_limit(seqlens_q, seqlens_k, device, min_visible=1)
    ...
    out, lse = flash_attn_varlen_func(
        ...
        max_seqlen_q=max(seqlens_q), max_seqlen_k=max(seqlens_k),
        causal=False, return_lse=True,
        col_limit=col_limit, window_size=(window_size_left, None),
    )
    ...
    for sq, sk in zip(seqlens_q, seqlens_k):
        rm_batch = col_limit[offset_q : offset_q + sq]
        out_b, _ = attention_col_limit_ref(
...
@maybe_fake_tensor_mode(USE_FAKE_TENSOR)
def test_col_limit_window_bwd(seqlens_q, seqlens_k, d, window_size_left, dtype):
    """Backward: col_limit + window_size_left vs materialized reference."""
    if SM120_NO_GQA:
        pass  # MHA only, no skip needed
    device = "cuda"
    nheads = 8
    ...
    for sq, sk in zip(seqlens_q, seqlens_k):
        ...
        out_b, _ = attention_col_limit_ref(q_b, k_b, v_b, rm_b, window_size_left=window_size_left)
        out_b.backward(dout_b)
        dq_ref[offset_q:offset_q+sq] = q_b.grad
        dk_ref[offset_k:offset_k+sk] = k_b.grad
        dv_ref[offset_k:offset_k+sk] = v_b.grad
